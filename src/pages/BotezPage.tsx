...
export function BotezPage() {
  ...
  return (
    <Layout>
      <Helmet>
        <title>Botez la Hotel Astoria Alba Iulia — Organizează botezul perfect</title>
        <meta name="description" content="Organizează botezul bebelușului tău la Hotel Astoria din Alba Iulia — sală cu pereți modulari, grădină interioară privată, camere elegante și beneficii exclusive." />
      </Helmet>
      ...
      <section className="relative h-screen min-h-[620px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          {heroSlides.map((src, i) => (
            <img
              key={src}
              src={src}
              alt="Botez Hotel Astoria"
              className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1400 ${
                i === slideIdx ? 'opacity-100' : 'opacity-0'
              }`}
            />
          ))}
          <div className="absolute inset-0 bg-gradient-to-br from-primary/50 to-[#0a140f]/68" />
        ...
        <div className="relative z-10 text-center px-6">
          <motion.p
            ...
            className="text-xs font-medium tracking-[0.28em] uppercase text-accent mb-4"
          >
          ...
          >
            Primul eveniment<br /><em className="italic text-accent-light">special al vieții sale</em>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Botezul este unul dintre cele mai emoționante momente din viața unei familii — la Hotel Astoria îl transformăm într-o amintire caldă și plină de bucurie.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <section className="bg-primary py-20 md:py-24 text-center">
        ...
        <p className="font-display text-xl md:text-2xl font-light italic text-accent max-w-[740px] mx-auto px-6 leading-relaxed">
          La <strong className="not-italic font-semibold text-accent-light">Hotel Astoria din Alba Iulia</strong> oferim un cadru elegant și primitor, perfect pentru a sărbători <strong className="not-italic font-semibold text-accent-light">primul eveniment important din viața copilului tău</strong>, alături de cei dragi.
        </p>
      </section>
      ...
      <section className="py-24 md:py-28 bg-bg" id="avantaje">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-3xl md:text-5xl font-light text-primary">
            Tot ce ai nevoie pentru un botez <em className="italic">de neuitat</em>
          </h2>
      ...
      <section className="py-24 md:py-28 bg-[#EDE7DC]" id="galerie">
        <div className="text-center mb-16 px-6">
          <span className="text-xs font-medium tracking-[0.25em] uppercase text-accent block mb-3">Galerie foto</span>
          <h2 className="font-display text-3xl md:text-5xl font-light text-primary">
            Clipe <em className="italic">autentice</em>
          </h2>
          <div className="w-12 h-px bg-accent mx-auto mt-5" />
        </div>
        <div className="grid grid-cols-2 md:grid-cols-12 gap-1 px-1 max-w-full">
          {galleryImages.map((src, i) => {
            ...
            return (
              <div
                key={src}
                className={`relative group cursor-pointer overflow-hidden ${spans[i]} h-[200px] md:h-[220px] ${i === 0 ? 'md:h-[440px]' : ''}`}
                onClick={() => openLb(i)}
              >
                <div className="absolute inset-0 bg-[#EDE7DC]" />
                <img
                  src={src}
                  alt={`Botez Hotel Astoria fotografie ${i + 1}`}
                  loading="lazy"
      ...
      {lbOpen && (
        ...
        >
          ...
          <img src={galleryImages[lbIdx]} alt="Galerie" className="max-w-[90vw] max-h-[88vh] object-contain border border-accent/20" />
      ...
      <section className="bg-primary py-24 md:py-28" id="beneficii">
        <div className="text-center mb-16 px-6">
          <span className="text-xs font-medium tracking-[0.25em] uppercase text-accent block mb-3">Pachet botez</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-[1100px] mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <h3 className="font-display text-lg font-semibold text-accent-light mb-7 pb-4 border-b border-accent/30">Incluse în pachet</h3>
            <ul className="space-y-0">
              {includedBenefits.map((b) => (
                <li key={b} className="py-2.5 text-sm font-light text-white/80 leading-relaxed border-b border-white/6 flex items-start gap-3.5" dangerouslySetInnerHTML={{ __html: '✦ ' + b }} />
              ))}
            </ul>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.15 }}>
            <h3 className="font-display text-lg font-semibold text-accent-light mb-7 pb-4 border-b border-accent/30">Servicii opționale — discount până la 90%</h3>
            <ul className="space-y-0">
              {optionalBenefits.map((b) => (
                <li key={b} className="py-2.5 text-sm font-light text-white/80 leading-relaxed border-b border-white/6 flex items-start gap-3.5">
                  <span className="text-accent text-[0.6rem] mt-1 shrink-0">✦</span> {b}
                </li>
              ))}
            </ul>
            <p className="font-display italic text-sm text-accent-light mt-5">* Serviciile opționale sunt disponibile cu discount de până la 90% față de prețul pieței.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 md:py-28 bg-bg">
        <div className="text-center mb-16 px-6">
          ...
          <h2 className="font-display text-3xl md:text-5xl font-light text-primary">5 motive să alegi <em className="italic">Hotel Astoria</em></h2>
          <div className="w-12 h-px bg-accent mx-auto mt-5" />
        </div>
      ...
      <section className="relative py-28 md:py-32 text-center bg-primary-light overflow-hidden">
        ...
        <div className="relative z-10 px-6">
          <span className="text-xs font-medium tracking-[0.25em] uppercase text-accent block mb-5">Următorul pas</span>
          <h2 className="font-display text-3xl md:text-5xl lg:text-6xl font-light text-white mb-4 leading-tight">Fă din acest moment<br />o amintire <em className="italic text-accent-light">de neuitat</em></h2>
          <p className="text-sm md:text-base font-light text-white/70 mb-12 leading-relaxed">Cere o ofertă personalizată și lasă-ne să organizăm împreună botezul perfect pentru micuțul tău.</p>
