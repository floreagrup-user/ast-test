import { Helmet } from 'react-helmet-async'
import { Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { Heart, Presentation, PartyPopper } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { Layout } from '@/components/layout/Layout'
import { ImageWithFallback } from '@/components/shared/ImageWithFallback'

const eventIcons = [Heart, Presentation, PartyPopper]
const eventImages = [
  'https://pub-8638b9dc92c2463b812e5fea5b32e051.r2.dev/general/hotel-astoria-locatie10.webp',
  'https://pub-8638b9dc92c2463b812e5fea5b32e051.r2.dev/general/hotel-astoria-locatie29.webp',
  'https://pub-8638b9dc92c2463b812e5fea5b32e051.r2.dev/general/hotel-astoria-locatie4.webp',
]

export function EventsPage() {
  const { t } = useTranslation()
  const sections = t('events.sections', { returnObjects: true }) as {
    title: string
    subtitle: string
    description: string
    imageAlt: string
  }[]

  return (
    <Layout>
      <Helmet>
        <title>{t('events.metaTitle')}</title>
        <meta name="description" content={t('events.metaDescription')} />
        <meta property="og:title" content={t('events.metaTitle')} />
        <meta property="og:description" content={t('events.metaDescription')} />
        <meta property="og:image" content="https://pub-8638b9dc92c2463b812e5fea5b32e051.r2.dev/general/hotel-astoria-locatie11.webp" />
      </Helmet>

      <section className="relative h-[50vh] min-h-[350px] flex items-end pb-12 overflow-hidden">
        <div className="absolute inset-0">
          <img
            src="https://pub-8638b9dc92c2463b812e5fea5b32e051.r2.dev/general/hotel-astoria-locatie11.webp"
            alt={t('events.heroTitle1')}
            className="w-full h-full object-cover"
            fetchPriority="high"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
        </div>
        <div className="relative container-xl z-10">
          <span className="inline-block text-xs font-semibold tracking-[0.2em] uppercase text-accent-light mb-3">
            {t('events.heroEyebrow')}
          </span>
          <h1 className="font-display text-4xl md:text-5xl font-normal text-white tracking-tight">
            {t('events.heroTitle1')} <em className="not-italic italic text-accent-light">{t('events.heroTitleItalic')}</em>
          </h1>
        </div>
      </section>

      <section className="py-16 md:py-24">
        <div className="container-xl">
          {sections.map((section, index) => {
            const Icon = eventIcons[index]
            return (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6 }}
                className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-20 last:mb-0 ${
                  index % 2 === 1 ? 'lg:[&>div:first-child]:order-2' : ''
                }`}
              >
                <div>
                  <div className="w-12 h-12 rounded-sm bg-accent/10 flex items-center justify-center mb-5">
                    <Icon className="w-6 h-6 text-accent" />
                  </div>
                  <p className="text-sm italic text-text-muted mb-2">{section.subtitle}</p>
                  <h2 className="font-display text-3xl md:text-4xl font-normal tracking-tight mb-5">
                    {section.title}
                  </h2>
                  <p className="text-text-muted leading-relaxed mb-8">{section.description}</p>
                  <Link
                    to="/contact"
                    className="inline-flex items-center bg-primary text-white font-medium px-8 py-3 rounded-sm hover:bg-primary-light transition-all duration-300"
                  >
                    {t('events.ctaRequest')}
                  </Link>
                </div>

                <div className="aspect-[4/3] rounded-sm overflow-hidden">
                  <ImageWithFallback src={eventImages[index]} alt={section.imageAlt} className="w-full h-full" />
                </div>
              </motion.div>
            )
          })}
        </div>
      </section>

      <section className="py-16 bg-primary text-white text-center">
        <div className="container-xl">
          <h2 className="font-display text-3xl md:text-4xl font-normal mb-4">
            {t('events.ctaTitle')}
          </h2>
          <p className="text-white/70 max-w-lg mx-auto mb-8">
            {t('events.ctaSubtitle')}
          </p>
          <Link
            to="/contact?subiect=eveniment"
            className="inline-flex items-center bg-accent text-primary font-medium px-10 py-4 rounded-sm hover:bg-accent-light transition-all duration-300 text-lg"
          >
            {t('events.ctaButton')}
          </Link>
        </div>
      </section>
    </Layout>
  )
}
